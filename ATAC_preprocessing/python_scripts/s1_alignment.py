...
def generate_job_scripts(input_table_path, template_path, config):
    ...
    for index, row in df.iterrows():
        ...
        job_content = job_content.replace("{{REF_GENOME}}", config['ref_genome'])
        job_content = job_content.replace("{{FASTQ_DIR}}", config['fastq_dir'])
        job_content = job_content.replace("{{BAM_DIR}}", config['bam_dir'])
        job_content = job_content.replace("{{REPORTS_DIR}}", config['reports_dir'])
        
        # Write job script
        job_script_path = Path("jobs") / f"s1_bwa_alignment_{sample_name}.sh"
...
def main():
    ...
    Main function
    """
    print("BWA Alignment Job Generator")
    print("=" * 40)
    
    # Configuration
    config = {
        'ref_genome': 'index/mm10',
        'fastq_dir': 'raw_data',
    ...
    template_script = 'base_scripts/s1_alignment.sh'
    ...
    if not os.path.exists(template_script):
        ...
        print("Please place your bash template script in the base_scripts/ directory.")
