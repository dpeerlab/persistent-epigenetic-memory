...
def generate_job_scripts(input_table_path, template_path, config):
    ...
    for index, row in df.iterrows():
        ...
        job_content = job_content.replace("{{BLACKLIST_PATH}}", config['blacklist_path'])
        
        # Write job script with s3_ prefix
        job_script_path = Path("jobs") / f"s3_cleaning_{sample_name}.sh"
...
def main():
    """
    Main function for S3 processing job generation
    """
    print("S3 Processing Job Generator")
    print("=" * 40)
    
    # Configuration
    config = {
        'bam_dir': 'intermediate_outputs/bam',
        'blacklist_path': '/data/peer/sotougl/review_2025/ATAC_2025/blacklist/mm10.blacklist.bed',
        'log_dir': 'logs'
    }
    
    # File paths
    input_table = 'sample_table.csv'
    template_script = 'base_scripts/s3_cleaning.sh'
    ...
    if not os.path.exists(template_script):
        ...
        print("Please place your bash template script 's3_cleaning.sh' in the base_scripts/ directory.")
    ...
    if job_scripts:
        ...
        print(f"\nSuccessfully generated {len(job_scripts)} S3 processing job scripts!")
    else:
